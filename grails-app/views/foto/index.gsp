		...
		<div id="list-foto" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="imageContentType" title="${message(code: 'foto.imageContentType.label', default: 'Image Content Type')}" />
				...
				<g:each in="${fotoInstanceList}" status="i" var="fotoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${fotoInstance.id}">${fieldValue(bean: fotoInstance, field: "image")}</g:link></td>
					
						<td>${fieldValue(bean: fotoInstance, field: "imageContentType")}</td>
